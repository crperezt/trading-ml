...
class SecCrawler():
    ...
    def __init__(self):
        ...
        self.company_list = pd.read_csv(join(FORMS_DATA_PATH, 'company_list.csv'))
    ...
    def parse_form4(self, start_date, end_date, returns=False):
        ...
        dataset_file = open(join(FORMS_DATA_PATH, 'dataset_norm_sector.csv'), 'a+')
        ...
        for i, c in enumerate(company_list.SYMBOL[951:]):
    ...
    def get_return_13(self, company, month, csector):
        ...
        for attempts in range (20):
            try:
                ...
                spy_ret_list = pd.read_json("https://api.tiingo.com/tiingo/daily/" + sector_ticker + "/prices?startDate=" + month_first + "&endDate=" + month_last + "&token=8a387055f2f4081b89abfc6b3044284e958f178e")
            except urllib2.HTTPError as err:
                if err.code == 429:
                    print "429 HTTP Error, sleeping for 20 minutes..."
                    time.sleep(1200)
                    print "Back online"
                elif err.code == 404:
        ...
        #print ret_list.tail()
        try:
            price_rel_1 = float(ret_list.adjClose[0])/float(spy_ret_list.adjClose[0])
            price_rel_30 = float(ret_list.adjClose[len(ret_list)-1])/float(spy_ret_list.adjClose[len(spy_ret_list)-1])
            ret = (float(price_rel_30) - float(price_rel_1))/float(price_rel_1)
        except:
            ret = 'None'
        return ret
...
if __name__ == '__main__':
    s = SecCrawler()
    s.parse_form4('2012-01', '2016-11',returns=True)
