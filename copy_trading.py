...
def run_models():
    ...
    #dataset.loc[dataset.RETURN.astype(np.float64) >= 0.00, 'RETURN_CAT'] = np.str('plus')
    #dataset.loc[dataset.RETURN.astype(np.float64) < 0.00, 'RETURN_CAT'] = np.str('minus')
    ...
    #clf_B = svm.SVC(C=1000,random_state = 2)
    ...
    joblib.dump(clf_A, 'test_model.pkl')
