...
from config import FORMS_DATA_PATH, HTTP_TIMEOUT, FAIL_LOG_FILE
import urllib2
    ...
    def save_in_directory(self, company_code, cik, priorto, doc_list,
        ...
        for j in range(len(doc_list)):
            ...
            for a in range(self.max_attempts):
                ...
                except urllib2.exceptions.ConnectTimeoutError:
                	print "HTTP Connections to EDGAR exceeded. Sleeping for 11 minutes..."
                	time.sleep(660)
                	print "Back online"
                except:
                    print "FAILED Unknown exception fetching doc list for " + cik
                else:
                    break
                finally:
        ...
        while more_docs:
            ...
            for attempts in range(self.max_attempts):
                ...
                #except requests.exceptions.ReadTimeout:
                    ...
                    print "FAILED Unknown exception fetching docs for " + cik
                    with open(FAIL_LOG_FILE, "a+") as fail_log:
                        fail_log.write(cik + '\n')
                else:
                    break
